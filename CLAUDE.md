...
- **Failure taxonomy**: every non-2xx response becomes a typed `ApiException` subclass, built by `Exceptions\ApiExceptionFactory` from the engine's `code` field first and the HTTP status second: `ValidationFailedException` (400/422), `NotFoundException` (404), `UnauthorizedException` (401/403), `TransientApiException` (429/5xx). Each answers `failureClass(): FailureClass` - `PERMANENT_ITEM`, `PERMANENT_CONFIG` or `TRANSIENT` - so a caller picks its retry policy without matching message text. Client-side validation implements the same `ClassifiedFailure` interface and reports `PERMANENT_ITEM` (`ValidationException`, and every `V2Exception` including `ImageUrl` and `Product::fromArray`). The raw code is on `$e->errorCode` (not `$e->code`, which `Exception` already uses for the status). `ErrorCode` is the engine's vocabulary, mirrored from its `docs/openapi-v2.yaml`; an older engine sends no code, so null always falls back to the status.
